<?php
    ...
    $qry3 = mysqli_query($con,"SELECT DISTINCT(enrolee.member_id) FROM enrolee LEFT JOIN eda ON enrolee.member_id = eda.eda_id WHERE eda.gender = 'MALE'");
    $countMale = mysqli_num_rows($qry3);
    $qry4 = mysqli_query($con,"SELECT DISTINCT(enrolee.member_id) FROM enrolee LEFT JOIN eda ON enrolee.member_id = eda.eda_id WHERE eda.gender = 'FEMALE'");
                    ...
                    <tbody>
                        <?php
                            ...
                            while ($date <= strtotime(date('m/d/Y'))) {
                                $sql = "SELECT * FROM enrolee WHERE application_date = '".date("m/d/Y",$date)."'";
                                $qry = mysqli_query($con,$sql);
                                $total_enrolee = mysqli_num_rows($qry);
